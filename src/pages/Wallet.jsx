...
const Wallet = () => {
    ...
    const [generatedCode, setGeneratedCode] = useState(''); // Code generated by Receiver
    ...
    const handleGenerateQR = (e) => {
        ...
        // Generate a 4-digit verification code for the Receiver to verify later
        ...
        const data = JSON.stringify({
            ...
            verificationCode: code // Embed code in QR for Sender to see
        });
        setQrData(data);
    };
    ...
    const handleSendPayment = (e) => {
        ...
        // No need to generate code here anymore, we use the one from scannedData
        updateBalance(scannedData.amount, `Sent to QR ID ${scannedData.id}`, 'debit');
        setConfirmationCode(scannedData.verificationCode);
    };

    const handleReceiveConfirmation = (e) => {
        e.preventDefault();
        if (receiverConfirmCode === generatedCode) {
            ...
            alert("Incorrect Verification Code! Ask Sender to check their screen.");
        }
    };
                ...
                <div style={{ background: 'var(--bg-card)', padding: '20px', borderRadius: '20px' }}>
                    ...
                    {!confirmationCode ? (
                        ...
                        <div style={{ textAlign: 'center' }}>
                            ...
                            <p>Tell this code to Receiver to verify:</p>
                            <div style={{ background: '#E8F5E9', padding: '20px', borderRadius: '10px', fontSize: '40px', fontWeight: 'bold', color: '#2E7D32', margin: '20px 0', letterSpacing: '5px' }}>
                                {scannedData.verificationCode}
                            </div>
                            <button onClick={() => { setMode('home'); setPaymentPin(''); setConfirmationCode(null); }} style={{ width: '100%', padding: '15px', borderRadius: '10px', border: 'none', background: '#9C27B0', color: 'white', fontWeight: 'bold' }}>Done</button>
                        </div>
                    )}
                </div>
            )}
